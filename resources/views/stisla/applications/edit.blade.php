...
@section('title', "Applications › Edit")
...
<div class="section-body">
    <div class="row no-gutter align-items-center">
        <div class="col-12 col-md-6 col-lg-6 d-none d-sm-block">
            <div class="float-left">
                ...
                <p class="section-lead">Edit application.</p>
            </div>
        </div>
    ...
    <form method="POST" action="{{ route('applications.update', $application) }}" class="ajaxForm form-group" enctype="multipart/form-data">
        @csrf
        @method('PATCH')
        <div class="form-group">
            <label for="name">Name</label>
            <input name="name" type="text" class="form-control" id="name" aria-describedby="nameHelp" placeholder="Enter application name" value="{{ $application->name }}">
            <small id="nameHelp" class="form-text text-danger"></small>
        </div>

        <div class="form-group">
            <label for="target">Target</label>
            <input name="target" type="text" class="form-control" id="target" aria-describedby="targetHelp" placeholder="Enter application target route" value="{{ $application->target }}">
        ...
        <div class="form-group">
            ...
            <select name="sub" class="form-control select2" id="sub" aria-describedby="subHelp" title="Select application parent">
                ...
                <option value="{{ $id }}" @selected($application->sub == $id)>{{ $name }}</option>
                @endforeach
            </select>
        ...
        <div class="form-group">
            ...
            <input name="icon" type="text" class="form-control" id="icon" aria-describedby="iconHelp" placeholder="Enter application icon" value="{{ $application->icon }}">
